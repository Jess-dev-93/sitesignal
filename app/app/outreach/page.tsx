...
function MiniScoreCard({ label, value }: { label: string; value: number | undefined }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 rounded-2xl border border-white/[0.08] bg-white/[0.035] px-4 py-4">
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-6xl space-y-6 p-4 pb-16 sm:p-6">
        ...
        <div className="space-y-8">
          ...
          >
            <div className="relative overflow-hidden border-b border-white/[0.07] bg-white/[0.03] px-6 py-5 sm:px-10">
              ...
              <div className="relative flex items-center gap-3">
                ...
                <div>
                  <p className="mb-0.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Target</p>
                  <h2 id="target-heading" className="text-lg font-bold text-white sm:text-xl">
                    {businessName}
                  </h2>
                </div>
              </div>
            ...
            <div className="px-6 py-5 sm:px-10">
              <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] px-4 py-3">
                  <dt className="mb-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Domain</dt>
                  <dd className="text-sm font-medium text-slate-200">{domain || <span className="text-slate-500">—</span>}</dd>
                </div>

                <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] px-4 py-3">
                  <dt className="mb-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Temperature</dt>
                  <dd>{leadTemp ? <TempBadge temp={leadTemp} /> : <span className="text-sm text-slate-500">—</span>}</dd>
                </div>

                <div className="rounded-2xl border border-white/[0.07] bg-white/[0.03] px-4 py-3">
                  <dt className="mb-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Opportunity</dt>
                  <dd className="text-sm font-medium text-slate-200">
                    {opportunityScore != null ? (
                      <span className="font-bold text-emerald-400">
                        {opportunityScore}<span className="ml-0.5 text-xs font-normal text-slate-500">/100</span>
                      </span>
                    ) : <span className="text-slate-500">—</span>}
                  </dd>
                </div>
          ...
          >
            <div className="relative overflow-hidden border-b border-white/[0.07] bg-white/[0.03] px-6 py-5 sm:px-10">
              ...
              <div className="relative flex items-center gap-3">
                ...
                <div>
                  <p className="mb-0.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Optional</p>
                  <h2 id="client-details-heading" className="text-lg font-bold text-white">
              ...
              <div className="relative overflow-hidden border-b border-white/[0.07] bg-white/[0.03] px-6 py-5 sm:px-10">
                ...
                <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="mb-0.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Audit Results</p>
                      <h2 id="audit-summary-heading" className="text-lg font-bold text-white">
                  ...
                  {hasAuditData && (
                    <div className="flex self-start gap-1 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-1 sm:self-center">
                      {(['mobile', 'desktop'] as const).map((m) => (
                        <button
                          ...
                          className={`rounded-xl px-4 py-1.5 text-xs font-semibold capitalize transition ${
                            auditMode === m ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
                          }`}
                        >
                          {m === 'mobile' ? '📱' : '🖥️'} {m}
                        </button>
              ...
              <div className="px-6 py-5 sm:px-10">
                {!hasAuditData ? (
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.06] p-6 text-center">
                    ...
                    <p className="mt-1 text-sm text-slate-400">Run an audit first to get outreach that references real findings.</p>
                    <button                       type="button"
                      onClick={() =>
                        router.push(
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-[28px] border border-white/[0.08] bg-white/[0.035] px-6 py-5 sm:px-10 shadow-[0_24px_60px_rgba(0,0,0,0.18)] backdrop-blur-sm">
            <div>
                  ...
                  <button
                    ...
                    className={`inline-flex items-center justify-center gap-2 rounded-2xl border px-5 py-3 text-sm font-semibold transition ${
                      addedToCallList
                        ...
                        : 'border-white/[0.10] bg-white/[0.04] text-slate-200 hover:bg-white/[0.08] disabled:cursor-not-allowed disabled:opacity-50'
                    }`}
                  >
                    {callListLoading ? (
                      <>
                ...
                {clientContact.contactPhone && (
                  <div className="mb-5 flex items-center gap-3 rounded-2xl border border-blue-500/20 bg-blue-500/[0.06] px-4 py-3">
                    ...
                    <div>
                      ...
                      {clientContact.contactName && (
                        <p className="text-xs text-slate-400">Ask for {clientContact.contactName}</p>
                      )}
                    </div>
                  </div>
                )}
